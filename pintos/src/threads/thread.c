...
{
  /* main and idle thread don't need these things */
  if (get_all_list_size() == 3) //main is creating the first proc
  { 
    t->wait_status = malloc(sizeof(struct wait_status));
    sema_init(&t->wait_status->dead, 0);
    sema_init(&t->wait_status->wait_load, 0);
  ...
  if (get_all_list_size() > 3) //proc is creating other proc
  {
    t->wait_status = malloc(sizeof(struct wait_status));
    sema_init(&t->wait_status->dead, 0);
    sema_init(&t->wait_status->wait_load, 0);
...
struct wait_status *get_tid_wait_status(tid_t tid)
{
  ...
  for (iter = list_begin(&thread_current()->children); iter != list_end(&thread_current()->children); iter = list_next(iter))
  {
    tid_wait_status = list_entry(iter, struct wait_status, elem);
    if (tid_wait_status->tid == tid)
    {
      return tid_wait_status;
    }
  }
  return NULL;
}
